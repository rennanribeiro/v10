...
for (const platform of ['html', 'react'] as const) {
  test(`${platform} keeps translated settings labels on one line while resizing`, async ({ page }) => {
    const query = new URLSearchParams({
      styling: 'css',
      skin: 'default',
      source: 'hls-4',
    ...
    await page.goto(`${SANDBOX_BASE}/${platform}-hls-video/?${query}`, { waitUntil: 'domcontentloaded' });

    const settings = page.locator('.media-button--settings').first();
    await expect(settings).toBeVisible({ timeout: 15_000 });
    await settings.click();

    const labelSelector = '.media-menu__item--submenu > :not(.media-icon):not(.media-menu__hint)';
    const englishLabel = page.locator(labelSelector).filter({ hasText: 'Captions' });
    await expect(englishLabel).toBeVisible({ timeout: 15_000 });
    const lineHeight = await englishLabel.evaluate((element) => element.getBoundingClientRect().height);

    const maxHeight = await englishLabel.evaluate(async (label) => {
      ...
      while (performance.now() - start < 600) {
        ...
        maxHeight = Math.max(maxHeight, label.getBoundingClientRect().height);
      }

      return maxHeight;
    });

    expect(maxHeight).toBeLessThanOrEqual(lineHeight + 1);
  });
}
