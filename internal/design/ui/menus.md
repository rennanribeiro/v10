...
- Keep base Menu independent of parent detection, navigation stacks, transition coordination, and player-setting selectors.
- Bind nested settings explicitly through `@videojs/core/dom/menu-transition`, the existing React `Menu.TransitionRoot` and `Menu.TransitionView` parts, or `@videojs/html/ui/menu-transition`. The binding derives the active view from child menu roots and owns forward/back policy.
...
- Optional shared coordinator: `packages/core/src/dom/ui/menu-transition/`
