...
export class SliderSegmentsElement extends MediaElement {
  ...
  protected getSegments(_min: number, _max: number): readonly SliderSegment[] {
    return this.#segments;
  }
  ...
  protected override update(_changed: PropertyValues): void {
    ...
    this.#core.setProps({ segments: this.getSegments(context.min, context.max) });
    ...
    for (const [index, rect] of nextRects.entries()) {
      const current = this.#clipPath.children[index];
      if (current !== rect) this.#clipPath.insertBefore(rect, current ?? null);
    }
