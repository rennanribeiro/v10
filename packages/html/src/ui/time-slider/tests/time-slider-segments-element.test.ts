...
describe('TimeSliderSegmentsElement', () => {
  it('renders chapter cues selected from the player store', async () => {
    ...
    expect(rects[1]?.style.getPropertyValue('--media-slider-segment-size')).toBe('25%');
    expect(Number.parseFloat(rects[3]?.style.getPropertyValue('--media-slider-segment-offset') ?? '')).toBeCloseTo(
      (100 / 120) * 100
    );
