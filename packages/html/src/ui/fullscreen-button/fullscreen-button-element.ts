...
import { selectFullscreen, type UIEvent } from '@videojs/core/dom';
...
export class FullscreenButtonElement extends MediaButtonElement<FullscreenButtonCore> {
  ...
  protected activate(state: MediaFullscreenState, event?: UIEvent): Promise<void> {
    if (this.core.shouldFocusContainer(event?.detail)) {
