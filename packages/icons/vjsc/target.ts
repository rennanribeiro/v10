...
export function createHtmlIconTarget(options: IconTargetOptions = {}): ComponentTarget<ComponentSchema> {
  ...
  return defineComponentTarget<ComponentSchema>()(({ element }) => {
    ...
    return {
      ...
      transforms: [createHtmlIconRegistrationTransform(family)],
      jsx: {
...
function createHtmlIconRegistrationTransform(family: string): ComponentTargetTransform {
  return {
    ...
    transform(context) {
      ...
      const registerIcons = imports.reference({ from: '@videojs/html/icons/element/register', name: 'registerIcons' });
      ...
      const entries = components.map((component) => {
        ...
        const imported = imports.reference({ from: source, name: `${iconNames(name).camel}Icon` });
