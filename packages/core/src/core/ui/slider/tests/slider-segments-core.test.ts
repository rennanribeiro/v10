...
import { getChapterSegments, SliderSegmentsCore } from '../slider-segments-core';
...
describe('SliderSegmentsCore', () => {
  ...
  it('does not highlight excluded ranges', () => {
    const core = new SliderSegmentsCore({ segments: [{ start: 0, end: 100, highlight: false }] });

    expect(core.getState('horizontal', 0, 100, 50).segments[0]?.highlighted).toBe(false);
  });
...
describe('getChapterSegments', () => {
  it('fills uncovered time with blank segments', () => {
    expect(
      getChapterSegments(
        [
          { startTime: 10, endTime: 20, text: 'Introduction' },
          { startTime: 30, endTime: 50, text: 'Main' },
        ],
        0,
        60
      )
      ...
      { start: 30, end: 50 },
      { start: 50, end: 60, highlight: false },
    ]);
  });

  it('does not render segments without chapter cues', () => {
    expect(getChapterSegments([], 0, 60)).toEqual([]);
  });
});
