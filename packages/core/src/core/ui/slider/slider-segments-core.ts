...
export class SliderSegmentsCore {
  ...
  getState(orientation: SliderState['orientation'], min: number, max: number, pointer?: number): SliderSegmentsState {
    ...
    const segments = this.#props.segments.flatMap(({ start, end, highlight = true }) => {
        ...
        ? [
            {
              offset,
              size,
              highlighted: highlight && pointer !== undefined && contains(pointer),
            },
          ]
        : [];
    });
...
/** Converts chapter cues to segments and fills uncovered time with blank ranges. */
export function getChapterSegments(cues: readonly MediaTextCue[], min: number, max: number): SliderSegment[] {
  ...
  for (const cue of cues) {
    ...
    if (!Number.isFinite(start) || !Number.isFinite(cueEnd) || cueEnd <= start) continue;

    if (start > end) segments.push({ start: end, end: start, highlight: false });
    segments.push({ start, end: cueEnd });
    end = Math.max(end, cueEnd);
  }

  if (segments.length > 0 && end < max) segments.push({ start: end, end: max, highlight: false });
  return segments;
}
