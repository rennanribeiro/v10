...
export class SliderChapterCore {
  getState(cues: readonly MediaTextCue[], time: number, max: number): SliderChapterState {
    const cue = findTimelineEntry(cues, time);
    const lastCue = cues[cues.length - 1];
    const active = cue && (time < cue.endTime || (cue === lastCue && time === cue.endTime && cue.endTime === max));
