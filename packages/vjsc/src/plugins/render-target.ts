...
import { renderTargetElement } from '../target/render';
...
/** Lower named canonical render targets without coupling them to component-path resolution. */
export function renderTargetPlugin(options: ComponentTargetPluginOptions): Plugin {
  return {
    name: 'vjsc:render-target',
    transform: {
      filter: { id: SCRIPT_ID, code: /(?:defineRenderTarget|\$[A-Za-z])/ },
      handler(code, id, transform) {
        ...
        const definitions = collectDefinitions(transform.ast);
        const imports = collectImportedNames(transform.ast);
        const canonical = collectCanonicalRoots(transform.ast, targets);
        const runtimeImports = createTargetModuleImports(transform.ast, transform.magicString);
        const typeImports = new TypeImports(transform.ast, transform.magicString);
        let changed = false;

        for (const definition of definitions.values()) {
          const resolved = resolveRenderTarget(definition.name, targets, definition.start);
          const replacement = renderDefinition(definition, resolved, runtimeImports, typeImports);

          transform.magicString.overwrite(definition.start, definition.end, replacement);
          changed = true;
        }
        ...
        walk(transform.ast, {
          enter(node, parent) {
            ...
            if (node.name.name !== '$render') {
              throw sourceError(
                `Unknown VJSC compiler directive \`${node.name.name}\`.\n` +
                  'Reason: compiler-only JSX props must be explicitly implemented and consumed.\n' +
                  'Recommendation: remove the prop or use a supported VJSC directive.',
                node.start
              );
            }
            ...
            if (parent?.type !== 'JSXOpeningElement' || !isCanonicalOpening(parent, canonical)) {
              throw sourceError(
                '`$render` can only be used on a canonical component or part.\n' +
                  ...
                  'Recommendation: move `$render` to the canonical component or part that owns the generated host.',
                node.start
              );
            }
            ...
            if (!name) {
              throw sourceError(
                `Cannot resolve render target \`${local}\`.\n` +
                  'Reason: `$render` must reference a local defineRenderTarget() declaration or a named relative import.\n' +
                  'Recommendation: define and export the target with defineRenderTarget(), then import that named export.',
                node.start
              );
            }

            const resolved = resolveRenderTarget(name, targets, node.start);

            lowerRenderDirective(code, parent, node, local, resolved.target, transform.magicString!);
            changed = true;
            this.skip();
          },
        });
...
function readDefinition(
  ...
  if (className?.type !== 'Literal' || typeof className.value !== 'string') {
    throw sourceError(
      'defineRenderTarget() requires one statically transformed style reference.\n' +
        ...
        'Recommendation: pass one direct value from an imported `.styles` module.',
...
function resolveRenderTarget(name: string, targets: readonly ComponentTarget[], pos: number): ResolvedRenderTarget {
  const matches = targets.flatMap((target) => {
    const element = target.renderTargets?.[name];
  ...
  if (matches.length === 0) {
    throw sourceError(
      `Component targets do not define render target \`${name}\`.\n` +
        'Reason: canonical render names require an explicit element contract for each framework.\n' +
        `Recommendation: add \`${name}\` to the selected target's renderTargets map.`,
      pos
    );
  }

  if (matches.length > 1) throw sourceError(`More than one component target defines render target \`${name}\`.`, pos);
...
function lowerRenderDirective(
  ...
  if (target.jsx.attributes === 'react') {
    if (
