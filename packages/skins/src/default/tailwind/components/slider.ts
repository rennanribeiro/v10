...
const previewContent = cn(
  'absolute [left:var(--preview-left,50%)] max-w-(--max-width)',
...
export const slider = {
  ...
  segments: cn(
    ...
    '**:data-[slot=slider-segment]:motion-safe:duration-200 **:data-[slot=slider-segment]:motion-safe:ease-out',
    // Horizontal
  ...
  preview: cn(
    'group/preview [--max-width:min(--spacing(48),100cqi)]',
  ...
  chapter: 'min-w-0 max-w-(--max-width) px-3 overflow-hidden text-ellipsis whitespace-nowrap empty:hidden',
};
