...
describe('generated VJSC source', () => {
  ...
  it('matches every transformed component and skin variant', async ({ expect }) => {
    ...
    expect(reactAudioSettingsMenu).toMatch(/<Menu\.Popup\s+keepMounted/);
    ...
    expect(reactCaptionsMenu).toContain('openWhen="multiple-options"');
    expect(reactCaptionsMenu).toMatch(/<Menu\.Popup\s+keepMounted/);
    expect(reactCaptionsMenu).not.toContain('useCaptionsOptions');
    expect(htmlCaptionsMenu).toContain('menu-for="__vjsc-id-<module>');
    expect(htmlCaptionsMenu).not.toContain('open-when');
