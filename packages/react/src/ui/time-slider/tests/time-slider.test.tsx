...
const mockTextTrackState = vi.hoisted(() => ({
  chaptersCues: [
    ...
    { startTime: 60, endTime: 100, text: 'Main' },
  ],
  thumbnailCues: [],
...
describe('TimeSlider compound', () => {
  ...
  it('renders chapter segments from text track cues', () => {
    const { Wrapper } = createPlayerWrapper();
    const props = { 'data-testid': 'segments', segments: [{ start: 0, end: 120 }] };
    const { container } = render(
      <Wrapper>
        <TimeSliderRoot>
          <TimeSliderSegments {...props} />
        </TimeSliderRoot>
      </Wrapper>
    );
    ...
    expect(rects[1]?.getAttribute('style')).toContain('--media-slider-segment-size: 25%');
    expect(rects[1]?.getAttribute('style')).toContain('--media-slider-segment-offset: 25%');
    expect(rects[3]?.getAttribute('style')).toContain('--media-slider-segment-offset: 83.33333333333334%');
