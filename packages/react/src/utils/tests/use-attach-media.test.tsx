...
describe('useAttachMedia', () => {
  ...
  it('leaves nothing attached when the adapter fails to attach', () => {
    ...
    // Nothing was recorded as attached, so the unmount React performs while unwinding has nothing to detach.
    expect(media.detach).not.toHaveBeenCalled();
  });
  ...
  it('survives StrictMode double invocation without re-attaching on later renders', () => {
    const media = createMedia();
    ...
    // Development StrictMode attaches, releases and attaches again on mount.
    expect(media.attach).toHaveBeenCalledTimes(2);
    expect(media.detach).toHaveBeenCalledTimes(1);
    ...
    expect(media.detach).toHaveBeenCalledTimes(2);
  });
});
