...
- *Media*: Pick what kind of media you have. The live presets accept streaming media only — `hls`, `dash`, or `mux-video` for `live-video`, and `mux-audio` for `live-audio`.
- *Source URL*: Define where your media comes from (or omit for a sample URL)
- *Install method*: How do you want to install your package? (CDN is HTML-only. The live presets ship CDN bundles for `live-video` with the default and minimal skins only — anything else is package-manager only.)
