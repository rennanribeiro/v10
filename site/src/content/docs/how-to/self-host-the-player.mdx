...
The skin bundles inline their CSS and apply it when the custom elements upgrade, so a mirrored copy renders correctly with no extra `<link>`. Alongside the bundles, `cdn/` publishes those styles as separate files for the one thing a script cannot do: style the page before that upgrade happens.
