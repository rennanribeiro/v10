...
The poster is a slotted `img` rather than an attribute, which gives the skin control over how it appears. `placeholdersrc` is the blurred stand-in shown while the real poster loads, Mux Player's `placeholder`.
</FrameworkCase>
...
Mux serves the poster from its `thumbnail` endpoint, so every Mux Player poster attribute becomes a query parameter you append: `thumbnail-time` is `?time=`, and the size, crop, rotation, and format options work the same way. See the <DocsLink slug="reference/poster">Poster</DocsLink> reference for the component itself.
...
```js
import { UIElement, selectTime } from '@videojs/html';
import { PlayerController } from '@videojs/html/video';

class MyElapsed extends UIElement {
  #time = new PlayerController(this, selectTime);
}
```
...
<FrameworkCase frameworks={["html"]}>
That's what <DocsLink slug="reference/html-create-player">`createPlayer`</DocsLink> is for. Hand it a feature list and you get back a configured `PlayerElement` class and typed `PlayerController`.
</FrameworkCase>
