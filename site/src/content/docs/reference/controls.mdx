...
Every Controls part receives `data-active-popup` while a named popover in the same player container is open. Use it when a control's own layout responds to that popup without inspecting unrelated descendants:

```css
.timeline[data-active-popup="volume"] {
