...
The container coordinates root menus, popovers, and tooltips within its boundary. Opening one closes the previously active grouped popup.

Named popovers are reflected on the container through `data-active-popup`. Descendant UI can use that state when its layout needs to respond without inspecting the popup's internal markup:

```css
.player-surface[data-active-popup="volume"] .timeline {
