...
```ts title="player.ts"
import { createPlayer, features, videoFeatures } from '@videojs/html';

export const { PlayerElement: LandscapePlayerElement } = createPlayer({
  features: [...videoFeatures, features.orientationLock],
});
```
</FrameworkCase>
...
export const { PlayerElement: PortraitPlayerElement } = createPlayer({
  features: [...videoFeatures, features.orientationLock({ type: 'portrait' })],
});
```
</FrameworkCase>
