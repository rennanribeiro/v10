...
- `create*` factories and mixins: `createPlayer`, `createI18n`
...
- **Utilities** — hooks, utilities, controllers, and mixins. Commonly used entries
  go at the top of the list; everything else follows.

The categories are flat by design, so don't nest a new subsection inside them. Mark framework-specific entries with `frameworks: ['react']` or `frameworks: ['html']` on the entry itself.
