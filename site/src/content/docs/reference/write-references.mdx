...
This guide covers how to create API reference pages — component references, media references, and util references (hooks, controllers, mixins) under `reference/` in the docs sidebar.
...
1. **Import** — render the public React export, the HTML registration module, and the HTML CDN note with `<ComponentImports>`.
...
For HTML readers the component also notes that no per-component CDN bundle exists and links to the <DocsLink slug="concepts/cdn">CDN</DocsLink> concept, so individual pages never make CDN claims of their own.
...
- `create*` factories and mixins: `createPlayer`, `createMediaAttachMixin`
...
## Create a media reference page

Media reference pages pair hand-written prose — source formats, provider quirks, behavior differences from a native `<video>` — with `<MediaReference media="..." />`, which renders the generated API tables.

Every media page opens with an `## Import` section rendered by `<MediaImports>`, placed after the intro prose and before the first body section:
...
`media` is the module name: the suffix of `@videojs/react/media/{media}` and `@videojs/html/media/{media}`, and of the CDN bundle `media/{media}.js`. `react` is the named export from the React subpath.

Unlike UI components, every media module ships a matching CDN bundle, so for HTML readers the component names that bundle and links the <DocsLink slug="concepts/cdn">CDN</DocsLink> concept. A unit test enforces the Import section on every page that renders `<MediaReference />`.

## Create a feature reference page

Feature reference pages document the feature modules passed to `createPlayer`. They open with an `## Import` section rendered by `<FeatureImports>`, placed after the intro prose and before `<FeatureReference />`:

```mdx
## Import

<FeatureImports feature="playback" />
```

`feature` takes the same key as `<FeatureReference>`. The component derives the export name (`playbackFeature`, importable from `@videojs/react` or `@videojs/html`) and reads the generated preset references to state which preset bundles include the feature, so membership never drifts from the build. A unit test enforces the section on every page that renders `<FeatureReference />`.

## Add to the sidebar

Open `site/src/docs.config.ts` and add your page to the appropriate category under **API Reference**:

- **Skins** — the packaged skins each preset ships. Prose-only pages today (no generated skin JSON), opening with `<SkinImports preset="..." react="..." />`
...
Skins, UI Components, Media Components, and Player Features are flat by design. Do not add nested subsections to them. Mark framework-specific entries with `frameworks: ['react']` or `frameworks: ['html']` on the entry itself.
