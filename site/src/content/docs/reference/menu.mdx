...
Transition panels use `data-view-state="active|inactive"`, `data-direction="forward|back"`, `data-starting-style`, and `data-ending-style`. Every panel has `data-menu-view`; the root has `data-menu-root-view`, while child panels have `data-submenu`. The outer content publishes `--media-menu-width`, `--media-menu-height`, `--media-menu-available-width`, and `--media-menu-available-height`. CSS controls the motion, duration, and easing. With reduced motion or no animation CSS, navigation completes without waiting for an animation.
