...
description: Mixin for custom media elements to register themselves with the player via context
---

import UtilReference from "@/components/docs/api-reference/UtilReference.astro";
import DocsLink from "@/components/docs/DocsLink.astro";

`MediaAttachMixin` creates a class that registers itself with the closest <DocsLink slug="reference/player">player</DocsLink> on connect. Apply it to custom media elements so they automatically wire to the store.
...
You only need `MediaAttachMixin` when your base class doesn't already register as a media element with the player. For example, elements that wrap a third-party player or use a canvas renderer.

For plain `<video>` and `<audio>` elements, the player tracks subtree changes automatically — no mixin needed.
