...
If none of them has a value, the title is empty and the component renders nothing. To
change the title while the player is running, call `setContentTitle`.
...
Audio players are the exception. Their controls never hide on their own, so the title
follows playback alone.
